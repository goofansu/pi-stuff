...
effort: high
tools: read, grep, find, ls, git-history
---
...
3. Trace relevant definitions, callers, tests, configuration, and documentation. Use `git-history` when history or rationale bears on the request. The trace is complete when each requested behavior is supported by a path through the code or identified as unresolved.
