...
* [`librarian.ts`](pi/extensions/librarian.ts) - Cross-repository code research subagent. Spawns an isolated `pi` process with kimi-coding/k2p5 (cost-efficient default) to search and read code across GitHub repositories using `gh` CLI. Invokable as a tool by the agent or directly via `/librarian`.
* [`oracle.ts`](pi/extensions/oracle.ts) - Second opinion reasoning subagent. Spawns an isolated `pi` process with openai-codex/gpt-5.2 for complex analysis, debugging, and review tasks. The agent can autonomously consult the oracle, or you can invoke it directly via `/oracle`.
