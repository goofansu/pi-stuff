...
- If an extension requires env variables, declare them in the header comment and warn via `ctx.ui.notify` in a `session_start` handler — see `web-search.ts` for the pattern.
