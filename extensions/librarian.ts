...
export default function librarianExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    parameters: Type.Object({
      query: Type.String({
        description:
          "The research question or investigation task for the Librarian",
      }),
    }),
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (result.exitCode !== 0 || !result.output) {
        const errorMsg =
          result.error || result.output || "Librarian failed with no output";
        return {
  ...
  pi.registerCommand("librarian", {
    description:
      "Search and read code across GitHub repositories (code-related tasks only)",
    handler: async (args, ctx: ExtensionContext) => {
      ...
      ctx.ui.notify(
        `Librarian researching: ${query.length > 60 ? query.slice(0, 57) + "..." : query}`,
        "info",
      );
