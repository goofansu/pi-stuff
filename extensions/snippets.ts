      ...
      const result = await ctx.ui.custom<{
        ...
        return {
          render(width: number) {
            ...
            // Title row
            const countInfo =
              codeBlocks.length > VISIBLE_ITEMS
                ? ` (${cursor + 1}/${codeBlocks.length})`
                : "";
            const titleLeft =
              " " +
              theme.fg("accent", theme.bold("Snippets")) +
              theme.fg("dim", countInfo);
            ...
            // Blank separator
            lines.push(
              padToWidth("", LIST_WIDTH) + theme.fg("borderMuted", "\u2502"),
            );
            ...
            if (currentBlock) {
              ...
              for (
                let i = previewScrollOffset;
                i < codeLines.length &&
                previewCodeLines.length < PREVIEW_LINES - 3;
                i++
              ) {
            ...
            for (let i = 0; i < VISIBLE_ITEMS; i++) {
              ...
              if (itemIndex < codeBlocks.length) {
                ...
                const cursorIndicator = isCopied
                  ? "✓"
                  : isCursor
                    ? "\u25B8"
                    : " ";
            ...
            const totalCodeLines = currentBlock
            ...
            if (totalCodeLines > PREVIEW_LINES - 3) {
              lines.push(
                " " +
                  ...
                  theme.fg(
                    "dim",
                    ` (${previewScrollOffset + 1}-${Math.min(previewScrollOffset + PREVIEW_LINES - 3, totalCodeLines)}/${totalCodeLines})`,
                  ),
              );
            }
            lines.push(border);
