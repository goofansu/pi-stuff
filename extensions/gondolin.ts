/**
 * Pi + Gondolin Sandbox (pi extension)
 *
 ...
 * the VM.
 *
 * Recommended use: run untrusted repositories safely by using the `--gondolin`
 * flag — all tool calls (file access, shell commands) are isolated inside the VM:
 *
 *   cd /path/to/untrusted/repo
 *   pi --gondolin
 *
 * Notes:
 *   - The VM is only started when the `--gondolin` flag is passed
 *   - The VM is started on `session_start` (and lazily if a tool is used before that)
 *   - User `!` commands are also executed inside the VM
 *   - Without `--gondolin`, all tools fall back to normal host behaviour
...
export default function (pi: ExtensionAPI) {
  ...
  // Register the --gondolin CLI flag; the VM only starts when this is set.
  pi.registerFlag(GONDOLIN_FLAG, {
    description:
      "Enable Gondolin micro-VM sandbox: tool calls (read/write/edit/bash) and ! commands run inside the VM",
    type: "boolean",
  });

  function isEnabled(): boolean {
    return pi.getFlag(GONDOLIN_FLAG) === true;
  }
  ...
  pi.registerTool({
    ...
    async execute(id, params, signal, onUpdate, ctx) {
      if (!isEnabled()) return localRead.execute(id, params, signal, onUpdate);
  ...
  pi.registerTool({
    ...localWrite,
    async execute(id, params, signal, onUpdate, ctx) {
      if (!isEnabled()) return localWrite.execute(id, params, signal, onUpdate);
      const activeVm = await ensureVm(ctx);
  ...
  pi.registerTool({
    ...localEdit,
    async execute(id, params, signal, onUpdate, ctx) {
      if (!isEnabled()) return localEdit.execute(id, params, signal, onUpdate);
      const activeVm = await ensureVm(ctx);
  ...
  pi.registerTool({
    ...localBash,
    async execute(id, params, signal, onUpdate, ctx) {
      if (!isEnabled()) return localBash.execute(id, params, signal, onUpdate);
      const activeVm = await ensureVm(ctx);
  ...
  // Run user `!` commands inside the VM too (only when enabled)
  pi.on("user_bash", (_event, _ctx) => {
    if (!isEnabled() || !vm) return;
